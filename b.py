...
class Encoder(nn.Module):
    def __init__(self, corpus):
        ...
        from Encoder import EncoderLayer as el
        self.layers = nn.ModuleList(el(d_embedding, n_heads, d_ff=2048) for _ in range(n_layers))# 编码器层数　
...
class Decoder(nn.Module):
    def __init__(self, corpus):
        ...
        from Decoder import DecoderLayer as dl
        self.layers = nn.ModuleList(dl(d_embedding, n_heads, d_ff=2048) for _ in range(n_layers))# 编码器层数　
